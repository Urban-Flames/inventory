// src/app/inventory/yearly/dry-items/actions.ts
'use server';
...
export async function generateYearlyDryItemsSummary(date: string) {
  console.log('[Server Action] generateYearlyDryItemsSummary initiated');
  ...
  try {
    ...
    const monthlySummaries = await sql`
      SELECT 
        ...
        avg_daily_usage,
        stock_cover_days
      FROM dry_items_monthly_summary
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO dry_items_yearly_summary (
          ...
          ${item.year_opening}, ${item.total_added}, ${item.total_kitchen}, ${item.total_bar},
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/yearly/dry-items');
...
export async function loadYearlyDryItemsSummary(date: string) {
  console.log('Loading yearly dry items summary for date:', date);

  try {
    const { yearStart, yearEnd, yearValue } = getYearRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM dry_items_yearly_summary
