// src/app/inventory/yearly/dry-items/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { dryItemsData } from '@/data/dry-items';
import { 
  generateYearlyDryItemsSummary, 
  loadYearlyDryItemsSummary,
  getYearRange
} from './actions';
import { 
  Calendar, 
...
export default function YearlyDryItemsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadYearlyDryItemsSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateYearlyDryItemsSummary(selectedDate);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            <span>Yearly Dry Items Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/yearly/dry-items/export">
            <button
                                ...
                                <tbody>
                                  {monthlyDataForItem.map((month: any) => (
                                    <tr key={month.monthStart} className="border-t border-gray-200">
                                      ...
                                      <td className="p-2 text-center text-purple-600">-{month.bar}</td>
