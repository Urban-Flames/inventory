// src/app/inventory/yearly/main-stock/actions.ts
'use server';
...
export async function generateYearlyMainStockSummary(date: string) {
  console.log('[Server Action] generateYearlyMainStockSummary initiated');
  ...
  try {
    ...
    const monthlySummaries = await sql`
      SELECT 
        ...
        avg_daily_usage,
        stock_cover_days
      FROM main_stock_monthly_summary
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO main_stock_yearly_summary (
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/yearly/main-stock');
...
export async function loadYearlyMainStockSummary(date: string) {
  console.log('Loading yearly main stock summary for date:', date);

  try {
    const { yearStart, yearEnd, yearValue } = getYearRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM main_stock_yearly_summary
