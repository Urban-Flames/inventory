// src/app/inventory/yearly/kitchen/export/page.tsx
'use client';

import { useState } from 'react';
import { loadYearlyKitchenSummary, getYearRange } from '../actions';
...
export default function YearlyKitchenExportPage() {
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        const exportData: ExportRow[] = result.data.map((item: any) => ({
          ...
          'Avg Monthly Sales': Number(item.avg_monthly_sales) || 0,
        ...
        const fileName = `Yearly_Kitchen_Report_${yearValue}.csv`;
        ...
        setMessage({
          type: 'success',
          text: `Yearly kitchen report for ${yearValue} exported successfully!`
        });
      } else {
        setMessage({
          type: 'info',
    ...
    <div className="p-4 sm:p-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold">Export Yearly Kitchen Report</h1>
            <p className="text-sm text-gray-500">Generate CSV report for yearly kitchen summary</p>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
            ...
            <ul className="text-sm text-blue-700 space-y-1.5">
              <li className="flex items-start gap-2">
                ...
                <span>Yearly summary for all kitchen items</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
              ...
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
                <span>Average monthly sales and stock cover months</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
                <span>Total variance for the year</span>
              </li>
            </ul>
          </div>
