// src/app/inventory/yearly/kitchen/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { initialKitchenStockData } from '@/data/kitchenStockItems';
import { 
  generateYearlyKitchenSummary, 
  loadYearlyKitchenSummary,
  getYearRange
} from './actions';
import { 
  Calendar, 
...
export default function YearlyKitchenPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadYearlyKitchenSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateYearlyKitchenSummary(selectedDate);
  ...
  const totalSales = yearlyData.reduce((sum, item) => sum + (Number(item.total_sales) || 0), 0);
  const totalWaste = yearlyData.reduce((sum, item) => sum + (Number(item.total_waste) || 0), 0);
  const totalReceived = yearlyData.reduce((sum, item) => sum + (Number(item.total_received) || 0), 0);
  const totalVariance = yearlyData.reduce((sum, item) => sum + (Number(item.total_variance) || 0), 0);
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            <span>Yearly Kitchen Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/yearly/kitchen/export">
            <button
                  ...
                  <React.Fragment key={item.item_id}>
                    ...
                    >
                      ...
                      <td className="p-2 sm:p-3 hidden lg:table-cell text-gray-600">
                        {Number(item.avg_monthly_sales) || 0}
                      </td>
                      <td className="p-2 sm:p-3">
                        {isOut ? (
                          ...
                          <div className="p-3 sm:p-4 border-t">
                                ...
                                <tbody>
                                  {monthlyDataForItem.map((month: any) => (
                                    <tr key={month.monthStart} className="border-t border-gray-200">
                                      ...
                                      <td className="p-2 text-center">{month.opening}</td>
                                      <td className="p-2 text-center text-green-600">+{month.received}</td>
                                      <td className="p-2 text-center text-blue-600">-{month.sales}</td>
                                      <td className="p-2 text-center text-red-600">-{month.waste}</td>
                                      <td className="p-2 text-center font-bold">{month.closing}</td>
                                      <td className="p-2 text-center text-amber-600">{month.variance}</td>
                                    </tr>
                                  ))}
                                  <tr className="border-t-2 border-gray-300 bg-gray-100 font-semibold">
                                    ...
                                    <td className="p-2 text-center text-blue-700">-{Number(item.total_sales) || 0}</td>
                            ...
                            <div className="mt-3 flex flex-wrap gap-4 text-xs">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="font-medium">{Number(item.avg_monthly_sales) || 0} {item.unit}</span>
                              </div>
                              <div className="flex items-center gap-1">
      ...
      {yearlyData.length > 0 && (
        <div className="text-xs text-gray-500 border-t pt-4 space-y-1">
          <p>Year Summary: Opening + Received - Sales - Waste = Closing</p>
          <p>Avg Monthly Sales = Total Sales ÷ Months with Sales</p>
          <p>Stock Cover Months = Closing Stock ÷ Avg Monthly Sales</p>
          <p className="text-amber-600">Items with variance show a variance tag. Click to see monthly details.</p>
