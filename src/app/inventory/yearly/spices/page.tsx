// src/app/inventory/yearly/spices/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { initialSpiceStockData } from '@/data/dailySpicesData';
import { 
  generateYearlySpicesSummary, 
  loadYearlySpicesSummary,
  getYearRange
} from './actions';
import { 
  Calendar, 
...
export default function YearlySpicesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadYearlySpicesSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateYearlySpicesSummary(selectedDate);
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            <span>Yearly Spices Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/yearly/spices/export">
            <button
                          ...
                          <div className="p-3 sm:p-4 border-t">
                                ...
                                <tbody>
                                  {monthlyDataForItem.map((month: any) => (
                                    <tr key={month.monthStart} className="border-t border-gray-200">
                                      ...
                                      <td className="p-2 text-center text-orange-600">-{month.production}</td>
                                      <td className="p-2 text-center text-purple-600">-{month.kitchen}</td>
                                      <td className="p-2 text-center text-red-600">-{month.used}</td>
                                  ...
                                  <tr className="border-t-2 border-gray-300 bg-gray-100 font-semibold">
                                    ...
                                    <td className="p-2 text-center text-orange-700">-{Number(item.total_production) || 0}</td>
                                    <td className="p-2 text-center text-purple-700">-{Number(item.total_kitchen) || 0}</td>
                                    <td className="p-2 text-center text-red-700">-{Number(item.total_used) || 0}</td>
                            ...
                            <div className="mt-3 flex flex-wrap gap-4 text-xs">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="font-medium">{Number(item.avg_monthly_usage) || 0} {item.unit}</span>
                              </div>
                              <div className="flex items-center gap-1">
      ...
      {yearlyData.length > 0 && (
        <div className="text-xs text-gray-500 border-t pt-4 space-y-1">
          <p>Year Summary: Opening + Added - Used = Closing</p>
