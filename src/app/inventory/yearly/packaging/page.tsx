// src/app/inventory/yearly/packaging/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { packagingItemsData } from '@/data/packaging-items';
import { 
  generateYearlyPackagingSummary, 
  loadYearlyPackagingSummary,
  getYearRange
} from './actions';
import { 
  Calendar, 
...
export default function YearlyPackagingPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadYearlyPackagingSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateYearlyPackagingSummary(selectedDate);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            <span>Yearly Packaging Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/yearly/packaging/export">
            <button
            ...
            {categories.filter(c => c !== 'All').map(category => (
              ...
              >
                ...
                <span className="text-[10px] opacity-60">
                  ({yearlyData.filter(item => item.category === category).length})
                </span>
              </button>
            ))}
          </div>
                                ...
                                <tbody>
                                  {monthlyDataForItem.map((month: any) => (
                                    <tr key={month.monthStart} className="border-t border-gray-200">
                                      ...
                                      <td className="p-2 text-center text-orange-600">-{month.kitchen}</td>
                                      <td className="p-2 text-center text-purple-600">-{month.production}</td>
                                  ...
                                  <tr className="border-t-2 border-gray-300 bg-gray-100 font-semibold">
                                    ...
                                    <td className="p-2 text-center text-purple-700">-{Number(item.total_production) || 0}</td>
