// src/app/inventory/yearly/packaging/actions.ts
'use server';
...
export async function generateYearlyPackagingSummary(date: string) {
  console.log('[Server Action] generateYearlyPackagingSummary initiated');
  ...
  try {
    ...
    const monthlySummaries = await sql`
      SELECT 
        ...
        avg_daily_usage,
        stock_cover_days
      FROM packaging_monthly_summary
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO packaging_yearly_summary (
          ...
          ${item.year_opening}, ${item.total_added}, ${item.total_kitchen}, ${item.total_production},
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/yearly/packaging');
...
export async function loadYearlyPackagingSummary(date: string) {
  console.log('Loading yearly packaging summary for date:', date);

  try {
    const { yearStart, yearEnd, yearValue } = getYearRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM packaging_yearly_summary
...
// Get yearly summary for a specific item
export async function getYearlyItemDetails(itemId: string, yearValue: number) {
  console.log('Getting yearly details for item:', itemId);

  try {
    const summary = await sql`
      SELECT * FROM packaging_yearly_summary
      WHERE item_id = ${itemId} AND year_value = ${yearValue};
    `;
    ...
    const monthlyBreakdown = await sql`
      ...
      WHERE item_id = ${itemId} 
        AND EXTRACT(YEAR FROM month_start_date) = ${yearValue}
