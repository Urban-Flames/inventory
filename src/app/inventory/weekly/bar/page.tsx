// src/app/inventory/weekly/bar/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { initialBarStockData } from '@/data/barStockItems';
import { 
  generateWeeklyBarSummary, 
  loadWeeklyBarSummary,
  getWeekRange,
  getWeekDates 
} from './actions';
import { 
...
const BAR_CATEGORIES: Record<string, string> = {
...
export default function WeeklyBarPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadWeeklyBarSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateWeeklyBarSummary(selectedDate);
  ...
  const totalSales = weeklyData.reduce((sum, item) => sum + (Number(item.total_sales) || 0), 0);
  const totalWaste = weeklyData.reduce((sum, item) => sum + (Number(item.total_waste) || 0), 0);
  ...
  const categories = [...new Set(weeklyData.map(item => item.category))];
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/weekly/bar/export">
            <button
                  ...
                  <React.Fragment key={item.item_id}>
                    ...
                    >
                      ...
                      <td className="p-2 sm:p-3 hidden sm:table-cell text-gray-500 text-xs">
                        {BAR_CATEGORIES[item.category] || item.category}
                      </td>
                              ...
                              <table className="w-full text-xs">
                                <thead className="bg-gray-100">
                                  <tr>
                                    ...
                                    <th className="p-2 text-center">Down Open</th>
                                    <th className="p-2 text-center">Down Add</th>
                                    <th className="p-2 text-center">Down Close</th>
                                    <th className="p-2 text-center">Fridge Open</th>
                                    <th className="p-2 text-center">Fridge Add</th>
                                    <th className="p-2 text-center">Fridge Close</th>
                                ...
                                <tbody>
                                  {dailyDataForItem.map((day: any) => (
                                    <tr key={day.date} className="border-t border-gray-200">
                                      ...
                                      <td className="p-2 text-center">{day.downOpen}</td>
                                      <td className="p-2 text-center text-green-600">+{day.downAdd}</td>
                                      <td className="p-2 text-center">{day.downClose}</td>
                                      <td className="p-2 text-center">{day.fridgeOpen}</td>
                                      <td className="p-2 text-center text-amber-600">+{day.fridgeAdd}</td>
                                      <td className="p-2 text-center">{day.fridgeClose}</td>
                                      <td className="p-2 text-center text-red-600">-{day.waste}</td>
                                      <td className="p-2 text-center font-bold text-blue-600">{day.totalSales}</td>
                                    </tr>
                                  ))}
      ...
      {weeklyData.length > 0 && (
        <div className="text-xs text-gray-500 border-t pt-4 space-y-1">
          <p>Week Summary: Opening + Added - Sales - Waste = Closing</p>
