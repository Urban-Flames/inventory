// src/app/inventory/weekly/bar/export/page.tsx
'use client';

import { useState } from 'react';
import { loadWeeklyBarSummary, getWeekRange } from '../actions';
...
export default function WeeklyBarExportPage() {
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        const exportData: ExportRow[] = result.data.map((item: any) => ({
          ...
          'Category': BAR_CATEGORIES[item.category] || item.category || '',
          'Unit': item.unit || '',
        ...
        const fileName = `Weekly_Bar_Report_${range.weekStart}_to_${range.weekEnd}.csv`;
        ...
        setMessage({
          type: 'success',
          text: `Weekly bar report for ${formatDisplayDate(range.weekStart)} to ${formatDisplayDate(range.weekEnd)} exported successfully!`
        });
      } else {
        setMessage({
          type: 'info',
    ...
    <div className="p-4 sm:p-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold">Export Weekly Bar Report</h1>
            <p className="text-sm text-gray-500">Generate CSV report for weekly bar summary</p>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
            ...
            <ul className="text-sm text-blue-700 space-y-1.5">
              <li className="flex items-start gap-2">
                ...
                <span>Weekly summary for all bar items</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
                <span>Opening, added, sales, waste, and closing totals</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
              ...
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
                <span>Category breakdown for better organization</span>
              </li>
            </ul>
          </div>
