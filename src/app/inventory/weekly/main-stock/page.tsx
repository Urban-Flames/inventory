// src/app/inventory/weekly/main-stock/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { initialMainStockData } from '@/data/mainStockItems';
import { 
  generateWeeklyMainStockSummary, 
  loadWeeklyMainStockSummary,
  getWeekRange,
  getWeekDates 
} from './actions';
import { 
...
export default function WeeklyMainStockPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadWeeklyMainStockSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateWeeklyMainStockSummary(selectedDate);
  ...
  const categories = ['All', ...new Set(weeklyData.map(item => item.category))];
  ...
  const totalProduction = weeklyData.reduce((sum, item) => sum + (Number(item.total_production) || 0), 0);
  const totalKitchen = weeklyData.reduce((sum, item) => sum + (Number(item.total_kitchen) || 0), 0);
  ...
  // Get category counts
  const categoryCounts = weeklyData.reduce((acc, item) => {
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            <span>Weekly Main Stock Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/weekly/main-stock/export">
            <button
          ...
          <div className="bg-white p-3 sm:p-4 rounded-lg border shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl sm:text-2xl font-bold text-purple-600">{totalProduction}</p>
              </div>
                ...
                return (
                  <React.Fragment key={item.item_id}>
                    ...
                    >
                      ...
                      <td className="p-2 sm:p-3 text-purple-600">-{formatNumber(Number(item.total_production) || 0, item.unit)}</td>
                      <td className="p-2 sm:p-3 text-blue-600">-{formatNumber(Number(item.total_kitchen) || 0, item.unit)}</td>
                      <td className="p-2 sm:p-3 text-orange-600">-{formatNumber(Number(item.total_used) || 0, item.unit)}</td>
                      ...
                      <td className="p-2 sm:p-3 hidden lg:table-cell text-gray-600">
                        {formatNumber(Number(item.avg_daily_usage) || 0, item.unit)}
                      </td>
                      <td className="p-2 sm:p-3">
                        {isOut ? (
                          ...
                          <div className="p-3 sm:p-4 border-t">
                                ...
                                <tbody>
                                  {dailyDataForItem.map((day: any) => (
                                    <tr key={day.date} className="border-t border-gray-200">
                                      ...
                                      <td className="p-2 text-center">{formatNumber(day.opening, item.unit)}</td>
                                      <td className="p-2 text-center text-green-600">+{formatNumber(day.added, item.unit)}</td>
                                      <td className="p-2 text-center text-purple-600">-{formatNumber(day.production, item.unit)}</td>
                                      <td className="p-2 text-center text-blue-600">-{formatNumber(day.kitchen, item.unit)}</td>
                                      <td className="p-2 text-center font-bold">{formatNumber(day.closing, item.unit)}</td>
                                    </tr>
                                  ))}
                                  <tr className="border-t-2 border-gray-300 bg-gray-100 font-semibold">
                                    ...
                                    <td className="p-2 text-center text-purple-700">-{formatNumber(Number(item.total_production) || 0, item.unit)}</td>
                            ...
                            <div className="mt-3 flex flex-wrap gap-4 text-xs">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="font-medium">{formatNumber(Number(item.avg_daily_usage) || 0, item.unit)} {item.unit}</span>
                              </div>
                              <div className="flex items-center gap-1">
      ...
      {weeklyData.length > 0 && (
        <div className="text-xs text-gray-500 border-t pt-4 space-y-1">
          <p>Week Summary: Opening + Added - (Production + Kitchen) = Closing</p>
