// src/app/inventory/weekly/main-stock/actions.ts
'use server';
...
export async function generateWeeklyMainStockSummary(date: string) {
  console.log('[Server Action] generateWeeklyMainStockSummary initiated');
  ...
  try {
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO main_stock_weekly_summary (
          ...
          ${weekStart}, ${weekEnd},
          ${item.week_opening}, ${item.total_added}, ${item.total_production}, ${item.total_kitchen},
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/weekly/main-stock');
...
export async function loadWeeklyMainStockSummary(date: string) {
  console.log('Loading weekly main stock summary for date:', date);

  try {
    const { weekStart, weekEnd } = getWeekRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM main_stock_weekly_summary
