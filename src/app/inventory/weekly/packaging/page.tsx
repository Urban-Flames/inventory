// src/app/inventory/weekly/packaging/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { packagingItemsData } from '@/data/packaging-items';
import { 
  generateWeeklyPackagingSummary, 
  loadWeeklyPackagingSummary,
  getWeekRange,
  getWeekDates 
} from './actions';
import { 
...
export default function WeeklyPackagingPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateWeeklyPackagingSummary(selectedDate);
...
<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
    ...
    <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
      <span>Weekly Packaging Report</span>
    </h1>
  </div>
  ...
  <div className="flex items-center gap-2 flex-wrap">
    ...
    {/* Export Button */}
    <Link href="/inventory/weekly/packaging/export">
      <button
            ...
            {categories.filter(c => c !== 'All').map(category => (
              ...
              >
                ...
                <span className="text-[10px] opacity-60">
                  ({weeklyData.filter(item => item.category === category).length})
                </span>
              </button>
            ))}
          </div>
