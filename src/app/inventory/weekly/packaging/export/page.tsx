// src/app/inventory/weekly/packaging/export/page.tsx
'use client';

import { useState } from 'react';
import { loadWeeklyPackagingSummary, getWeekRange } from '../actions';
...
export default function WeeklyPackagingExportPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      const result = await loadWeeklyPackagingSummary(selectedDate);
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        // Fix: Await the getWeekRange call
        const range = await getWeekRange(selectedDate);
        const fileName = `Weekly_Packaging_Report_${range.weekStart}_to_${range.weekEnd}.csv`;
        ...
        setMessage({
          type: 'success',
          text: `Weekly packaging report for ${formatDisplayDate(range.weekStart)} to ${formatDisplayDate(range.weekEnd)} exported successfully!`
        });
      } else {
        setMessage({
          type: 'info',
  ...
  return (
    <div className="p-4 sm:p-6 max-w-2xl mx-auto">
      <Link 
        href="/inventory/weekly/packaging"
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold">Export Weekly Packaging Report</h1>
            <p className="text-sm text-gray-500">Generate CSV report for weekly packaging summary</p>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
            ...
            <ul className="text-sm text-blue-700 space-y-1.5">
              <li className="flex items-start gap-2">
                ...
                <span>Weekly summary for all packaging items</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
              ...
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
                <span>Kitchen and Production usage breakdown</span>
              </li>
            </ul>
          </div>
