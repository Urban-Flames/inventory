// src/app/inventory/weekly/dry-items/export/page.tsx
'use client';

import { useState } from 'react';
import { loadWeeklyDryItemsSummary, getWeekRange } from '../actions';
...
export default function WeeklyDryItemsExportPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        const fileName = `Weekly_Dry_Items_Report_${range.weekStart}_to_${range.weekEnd}.csv`;
        ...
        setMessage({
          type: 'success',
          text: `Weekly dry items report for ${formatDisplayDate(range.weekStart)} to ${formatDisplayDate(range.weekEnd)} exported successfully!`
        });
      } else {
        setMessage({
          type: 'info',
  ...
  return (
    <div className="p-4 sm:p-6 max-w-2xl mx-auto">
      <Link 
        href="/inventory/weekly/dry-items"
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold">Export Weekly Dry Items Report</h1>
            <p className="text-sm text-gray-500">Generate CSV report for weekly dry items summary</p>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
            ...
            <ul className="text-sm text-blue-700 space-y-1.5">
              <li className="flex items-start gap-2">
                ...
                <span>Weekly summary for all dry items</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
