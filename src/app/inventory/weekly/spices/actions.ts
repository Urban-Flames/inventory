// src/app/inventory/weekly/spices/actions.ts
'use server';
...
export async function generateWeeklySpicesSummary(date: string) {
  console.log('[Server Action] generateWeeklySpicesSummary initiated');
  ...
  try {
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO spices_weekly_summary (
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/weekly/spices');
...
export async function loadWeeklySpicesSummary(date: string) {
  console.log('Loading weekly spices summary for date:', date);

  try {
    const { weekStart, weekEnd } = getWeekRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM spices_weekly_summary
