// src/app/inventory/weekly/kitchen/export/page.tsx
'use client';

import { useState } from 'react';
import { loadWeeklyKitchenSummary, getWeekRange } from '../actions';
...
export default function WeeklyKitchenExportPage() {
  ...
  const getStockStatus = (item: any): string => {
    ...
    if (closingStock <= 10) return 'Low Stock';
    return 'OK';
  };
  ...
  const handleExport = async () => {
    ...
    try {
      const result = await loadWeeklyKitchenSummary(selectedDate);
      ...
      if (result.success && result.data && result.data.length > 0) {
        const exportData: ExportRow[] = result.data.map((item: any) => ({
          ...
          'Total Received': Number(item.total_received) || 0,
          'Total Sales': Number(item.total_sales) || 0,
          'Total Waste': Number(item.total_waste) || 0,
          'Week Closing': Number(item.closing_stock) || 0,
          'Avg Daily Sales': Number(item.avg_daily_sales) || 0,
          'Stock Cover Days': Number(item.stock_cover_days) || 0,
          'Total Variance': Number(item.total_variance) || 0,
        ...
        const fileName = `Weekly_Kitchen_Report_${range.weekStart}_to_${range.weekEnd}.csv`;
        ...
        setMessage({
          type: 'success',
          text: `Weekly kitchen report for ${formatDisplayDate(range.weekStart)} to ${formatDisplayDate(range.weekEnd)} exported successfully!`
        });
      } else {
        setMessage({
          type: 'info',
    ...
    <div className="p-4 sm:p-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold">Export Weekly Kitchen Report</h1>
            <p className="text-sm text-gray-500">Generate CSV report for weekly kitchen summary</p>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
            ...
            <ul className="text-sm text-blue-700 space-y-1.5">
              <li className="flex items-start gap-2">
                ...
                <span>Weekly summary for all kitchen items</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
                <span>Opening, received, sales, waste, and closing totals</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
                <span>Average daily sales and stock cover days</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600">•</span>
                <span>Total variance for the week</span>
              </li>
            </ul>
          </div>
