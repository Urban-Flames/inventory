// src/app/inventory/weekly/kitchen/actions.ts
'use server';
...
export async function generateWeeklyKitchenSummary(date: string) {
  console.log('[Server Action] generateWeeklyKitchenSummary initiated');
  ...
  try {
    ...
    for (const [itemId, itemData] of itemMap) {
      ...
      if (itemData.daily_data.length > 0) {
        itemData.week_opening = itemData.daily_data[0].morningStock;
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO kitchen_weekly_summary (
          ...
          ${weekStart}, ${weekEnd},
          ${item.week_opening}, ${item.total_received}, ${item.total_waste},
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/weekly/kitchen');
...
export async function loadWeeklyKitchenSummary(date: string) {
  console.log('Loading weekly kitchen summary for date:', date);

  try {
    const { weekStart, weekEnd } = getWeekRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM kitchen_weekly_summary
