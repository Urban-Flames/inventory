// src/app/inventory/weekly/kitchen/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { initialKitchenStockData } from '@/data/kitchenStockItems';
import { 
  generateWeeklyKitchenSummary, 
  loadWeeklyKitchenSummary,
  getWeekRange,
  getWeekDates 
} from './actions';
import { 
...
export default function WeeklyKitchenPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateWeeklyKitchenSummary(selectedDate);
  ...
  const totalReceived = weeklyData.reduce((sum, item) => sum + (Number(item.total_received) || 0), 0);
  const totalVariance = weeklyData.reduce((sum, item) => sum + (Number(item.total_variance) || 0), 0);
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            <span>Weekly Kitchen Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/weekly/kitchen/export">
            <button
      ...
      {weeklyData.length > 0 && (
        <div className="text-xs text-gray-500 border-t pt-4 space-y-1">
          <p>Week Summary: Opening + Received - Sales - Waste = Closing</p>
