// src/app/inventory/monthly/dry-items/actions.ts
'use server';
...
export async function generateMonthlyDryItemsSummary(date: string) {
  console.log('[Server Action] generateMonthlyDryItemsSummary initiated');
  ...
  try {
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO dry_items_monthly_summary (
          ...
          ${monthStart}, ${monthEnd},
          ${item.month_opening}, ${item.total_added}, ${item.total_kitchen}, ${item.total_bar},
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/monthly/dry-items');
...
export async function loadMonthlyDryItemsSummary(date: string) {
  console.log('Loading monthly dry items summary for date:', date);

  try {
    const { monthStart, monthEnd, monthName } = getMonthRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM dry_items_monthly_summary
