// src/app/inventory/monthly/packaging/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { packagingItemsData } from '@/data/packaging-items';
import { 
  generateMonthlyPackagingSummary, 
  loadMonthlyPackagingSummary,
  getMonthRange
} from './actions';
import { 
  Calendar, 
...
export default function MonthlyPackagingPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadMonthlyPackagingSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateMonthlyPackagingSummary(selectedDate);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            <span>Monthly Packaging Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/monthly/packaging/export">
            <button
            ...
            {categories.filter(c => c !== 'All').map(category => (
              ...
              >
                ...
                <span className="text-[10px] opacity-60">
                  ({monthlyData.filter(item => item.category === category).length})
                </span>
              </button>
            ))}
          </div>
                                ...
                                <tbody>
                                  {dailyDataForItem.map((day: any) => (
                                    <tr key={day.date} className="border-t border-gray-200">
                                      ...
                                      <td className="p-2 text-center text-purple-600">-{day.production}</td>
