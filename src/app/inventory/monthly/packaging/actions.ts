// src/app/inventory/monthly/packaging/actions.ts
'use server';
...
export async function generateMonthlyPackagingSummary(date: string) {
  console.log('[Server Action] generateMonthlyPackagingSummary initiated');
  ...
  try {
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO packaging_monthly_summary (
          ...
          ${monthStart}, ${monthEnd},
          ${item.month_opening}, ${item.total_added}, ${item.total_kitchen}, ${item.total_production},
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/monthly/packaging');
...
export async function loadMonthlyPackagingSummary(date: string) {
  console.log('Loading monthly packaging summary for date:', date);

  try {
    const { monthStart, monthEnd, monthName } = getMonthRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM packaging_monthly_summary
