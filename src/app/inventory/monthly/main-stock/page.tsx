// src/app/inventory/monthly/main-stock/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { initialMainStockData } from '@/data/mainStockItems';
import { 
  generateMonthlyMainStockSummary, 
  loadMonthlyMainStockSummary,
  getMonthRange
} from './actions';
import { 
  Calendar, 
...
export default function MonthlyMainStockPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadMonthlyMainStockSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateMonthlyMainStockSummary(selectedDate);
  ...
  const totalAdded = monthlyData.reduce((sum, item) => sum + (Number(item.total_added) || 0), 0);
  const totalProduction = monthlyData.reduce((sum, item) => sum + (Number(item.total_production) || 0), 0);
  const totalKitchen = monthlyData.reduce((sum, item) => sum + (Number(item.total_kitchen) || 0), 0);
  const totalUsed = monthlyData.reduce((sum, item) => sum + (Number(item.total_used) || 0), 0);
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            <span>Monthly Main Stock Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/monthly/main-stock/export">
            <button
                  ...
                  <React.Fragment key={item.item_id}>
                    ...
                    >
                      ...
                      <td className="p-2 sm:p-3 hidden lg:table-cell text-gray-600">
                        {formatNumber(Number(item.avg_daily_usage) || 0, item.unit, true)}
                      </td>
                      <td className="p-2 sm:p-3">
                        {isOut ? (
                          ...
                          <div className="p-3 sm:p-4 border-t">
                                ...
                                <tbody>
                                  {dailyDataForItem.map((day: any) => (
                                    <tr key={day.date} className="border-t border-gray-200">
                                      ...
                                      <td className="p-2 text-center text-purple-600">{formatNumber(day.production, item.unit, true)}</td>
                            ...
                            <div className="mt-3 flex flex-wrap gap-4 text-xs">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="font-medium">{formatNumber(Number(item.avg_daily_usage) || 0, item.unit, true)} {item.unit}</span>
                              </div>
                              <div className="flex items-center gap-1">
      ...
      {monthlyData.length > 0 && (
        <div className="text-xs text-gray-500 border-t pt-4 space-y-1">
          <p>Month Summary: Opening + Added - (Production + Kitchen) = Closing</p>
          ...
          <p className="text-xs text-gray-400">Note: Production, Used, and Avg Daily Usage values are shown with 4 decimal places</p>
        </div>
      )}
    </div>
  );
