// src/app/inventory/monthly/bar/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { initialBarStockData } from '@/data/barStockItems';
import { 
  generateMonthlyBarSummary, 
  loadMonthlyBarSummary,
  getMonthRange
} from './actions';
import { 
  Calendar, 
...
export default function MonthlyBarPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const result = await loadMonthlyBarSummary(selectedDate);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateMonthlyBarSummary(selectedDate);
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/inventory/monthly/bar/export">
            <button
      ...
      {monthlyData.length > 0 && (
        <div className="text-xs text-gray-500 border-t pt-4 space-y-1">
          <p>Month Summary: Opening + Added - Sales - Waste = Closing</p>
