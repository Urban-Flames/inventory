// src/app/inventory/monthly/bar/actions.ts
'use server';
...
export async function generateMonthlyBarSummary(date: string) {
  console.log('[Server Action] generateMonthlyBarSummary initiated');
  ...
  try {
    ...
    for (const [itemId, itemData] of itemMap) {
      ...
      // Get month opening (first day's total stock)
      if (itemData.daily_data.length > 0) {
        itemData.month_opening = itemData.daily_data[0].totalStock;
        itemData.month_closing = itemData.daily_data[itemData.daily_data.length - 1].totalStock;
      }
    ...
    for (const item of summaryData) {
      await sql`
        INSERT INTO bar_monthly_summary (
          ...
          ${monthStart}, ${monthEnd},
          ${item.month_opening}, ${item.total_added}, ${item.total_waste},
    ...
    await sql`COMMIT`;
    revalidatePath('/inventory/monthly/bar');
...
export async function loadMonthlyBarSummary(date: string) {
  console.log('Loading monthly bar summary for date:', date);

  try {
    const { monthStart, monthEnd, monthName } = getMonthRangeInternal(date);
    
    const summary = await sql`
      SELECT * FROM bar_monthly_summary
