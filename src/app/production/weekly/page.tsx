// src/app/production/weekly/page.tsx
'use client';

import React, { useState, useEffect } from 'react';
import { 
  generateWeeklyProductionSummary, 
  loadWeeklyProductionSummary,
  getWeekRange,
  getWeekDates 
} from './actions';
import { 
...
export default function WeeklyProductionPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateWeeklyProductionSummary(selectedDate);
  ...
  const categories = ['All', ...new Set(weeklyData.map(item => item.main_stock_item_name?.split(' ')[0] || 'Other'))];

  // Calculate summary statistics
  const totalItems = weeklyData.length;
  const totalProcessed = weeklyData.reduce((sum, item) => sum + (Number(item.total_quantity_processed) || 0), 0);
  const totalYield = weeklyData.reduce((sum, item) => sum + (Number(item.total_actual_yield) || 0), 0);
  const totalIssued = weeklyData.reduce((sum, item) => sum + (Number(item.total_kitchen_issued) || 0), 0);
  const totalRemaining = weeklyData.reduce((sum, item) => sum + (Number(item.total_remaining) || 0), 0);
    ...
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            <span>Weekly Production Report</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Export Button */}
          <Link href="/production/weekly/export">
            <button
      ...
      {weeklyData.length > 0 ? (
                        ...
                        <div>
                          ...
                          {hasRemaining && (
                            <span className="ml-2 text-[10px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded font-medium">
                              {Number(item.total_remaining)} remaining
                            </span>
                          )}
                        </div>
                      </td>
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg border">
          ...
          <h3 className="text-lg font-semibold text-gray-700 mb-2">No Weekly Production Data Available</h3>
